...
import EditSQL.data_util.sql_util as sql_util
import EditSQL.data_util.vocabulary as vocab
from EditSQL.data_util.vocabulary import EOS_TOK, UNK_TOK
import EditSQL.data_util.tokenizers
...
from EditSQL.data_util.utterance import ANON_INPUT_KEY
...
import EditSQL.data_util.atis_batch
...
class SchemaInteractionATISModel(ATISModel):
    ...
    def train_step(self, interaction, max_generation_length, snippet_alignment_probability=1.):
        ...
        for utterance_index, utterance in enumerate(interaction.gold_utterances()):
            ...
            gold_query_weights = utterance.gold_query_weights() # learning weight on each gold query token
            ...
            if len(gold_query) <= max_generation_length and len(previous_query) <= max_generation_length:
                prediction = self.predict_turn(final_utterance_state,
                                               ...
                                               training=True,
                                               gold_query_weights=gold_query_weights)
    ...
    def spider_single_turn_encoding(self, interaction, max_generation_length):
        ...
        # from predict_turn
        if self.params.use_encoder_attention:
            schema_attention = self.utterance2schema_attention_module(torch.stack(schema_states,dim=0), utterance_states).vector # input_value_size x len(schema)
            utterance_attention = self.schema2utterance_attention_module(torch.stack(utterance_states,dim=0), schema_states).vector # schema_value_size x len(input)
            ...
            new_input_hidden_states = torch.cat([torch.stack(utterance_states, dim=1), utterance_attention], dim=0) # (input_value_size+schema_value_size) x len(input)
            input_hidden_states = list(torch.split(new_input_hidden_states, split_size_or_sections=1, dim=1))
            input_hidden_states = [input_hidden_state.squeeze() for input_hidden_state in input_hidden_states]
