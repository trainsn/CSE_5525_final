...
def real_user_interactive_parsing_session( user, input_item, hyp, bool_verbal=False):
        ...
        while len(err_su_pointer_pairs):    # for each potential erroneous unit
            ...
            # question generation
            question, cheat_sheet = question_generator.question_generation(su, hyp.tag_seq, pointer)
            if len(question):
                ...
                if cheat_sheet[user_feedback][0]:   # user affirms the decision
                    ...
                    if cheat_sheet[user_feedback][1] == 0:
                        ...
                        # present options
                        opt_question, opt_answer_sheet, sel_none_of_above = question_generator.option_generation(
...
def ProcessQuestion():
    global input_item
    with torch.no_grad():
        question = request.get_json()['question'].split()
