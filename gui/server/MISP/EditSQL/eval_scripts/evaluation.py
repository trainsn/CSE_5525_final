...
def eval_and_or(pred, label):
    ...
    try:
        ...
        print((type(pred_ao), pred_ao))
        print((type(label_ao), label_ao))
        raise Exception
...
def print_scores(scores, etype):
    ...
    if etype in ["all", "exec"]:
        ...
        print("{:20} {:<20.4f} {:<20.4f} {:<20.4f} {:<20.4f} {:<20.4f}".format("execution", *this_scores))

    if etype in ["all", "match"]:
        print('\n====================== EXACT MATCHING ACCURACY =====================')
        exact_scores = [scores[level]['exact'] for level in levels]
        print("{:20} {:<20.4f} {:<20.4f} {:<20.4f} {:<20.4f} {:<20.4f}".format("exact match", *exact_scores))
        print('\n---------------------PARTIAL MATCHING ACCURACY----------------------')
        for type_ in partial_types:
            this_scores = [scores[level]['partial'][type_]['acc'] for level in levels]
            print("{:20} {:<20.4f} {:<20.4f} {:<20.4f} {:<20.4f} {:<20.4f}".format(type_, *this_scores))
...
def evaluate(gold, predict, db_dir, etype, kmaps, bool_verbal=True, bool_predict_file=True):
    ...
    for p, g in zip(plist, glist):
        ...
        if bool_verbal:
            print(("g_str: {}\ng_sql: {}\np_str: {}\np_sql: {}\n".format(g_str, g_sql, p_str, p_sql)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--close_verbal', action='store_true', default=False)
    ...
    evaluate(gold, pred, db_dir, etype, kmaps, bool_verbal=not args.close_verbal)